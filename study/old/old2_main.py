...
class DataAnalysisAgent:
    ...
    def create_report(self, json_input: str) -> str:
        ...
        try:
            ...
            payload = json.loads(json_input) if isinstance(json_input, str) else (
                json_input if isinstance(json_input, dict) else {})
    ...
    if st.button("분석 시작 (텍스트)"):
        if query:
            ...
            with st.spinner("분석 중..."):
                ...
                handled, msg = parse_report_request_and_execute(full_prompt, st.session_state.agent,
                                                                masked_df=masked_df)
                if handled:
                    st.write(msg)
                else:
                    ...
                    if pdfs:
                        ...
                        for p in pdfs:
                            with open(p, 'rb') as fh:
                                st.download_button(f"{p.name} 다운로드", data=fh.read(), file_name=p.name,
